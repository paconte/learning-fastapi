...
@pytest.fixture(scope="module")
async def mock_db(collection: Dict[int, Product], mock_products:List[ProductIn]) -> Database:
...
@pytest.mark.asyncio
async def test_post(client: httpx.AsyncClient, mock_products: List[ProductIn]) -> None:
    for product in mock_products:
...
@pytest.mark.asyncio
async def test_get_all(client: httpx.AsyncClient, mock_products: List[ProductIn]) -> None:
    response = await client.get("/products")
    assert response.status_code == 200
...
@pytest.mark.asyncio
async def test_get_single(client: httpx.AsyncClient, mock_products: List[ProductIn]) -> None:
    for i, product in enumerate(mock_products):
...
@pytest.mark.asyncio
async def test_get_wrong(client: httpx.AsyncClient, mock_products: List[ProductIn]) -> None:
    url = f"/products/{str(len(mock_products) + 1)}"
    response = await client.get(url)
    assert response.status_code == 404
    assert response.json()["detail"] == "Product with supplied ID does not exist"


@pytest.mark.asyncio
async def test_update(client: httpx.AsyncClient, mock_products: List[ProductIn]) -> None:
    response = await client.get("/products")
    print(response.json())
    for i, product in enumerate(mock_products):
        url = f"/products/{str(i)}"
...
@pytest.mark.asyncio
async def test_delete_product(client: httpx.AsyncClient, mock_products: List[ProductIn]) -> None:
...
@pytest.mark.asyncio
async def test_delete_wrong_product(client: httpx.AsyncClient, mock_products: List[ProductIn]) -> None:
    url = f"/products/{str(len(mock_products) + 1)}"
    response = await client.delete(url)
    assert response.status_code == 404
